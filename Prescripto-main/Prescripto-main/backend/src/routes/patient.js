...
router.post('/access-requests/:doctorId/reject', async (req, res) => {
  try {
    ...
    const reqIndex = patient.accessRequests.findIndex(
      (r) => r.doctorId.toString() === doctorId
    );
    ...
    patient.accessRequests[reqIndex].respondedAt = new Date();

    await patient.save();

    return res.json({ message: 'Doctor access rejected' });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
});
